...
# This function takes in allmort (simulation of attributable mortality/incidence) 
# and total mort (simulation of total mortality/incidence) to calculate 
# "reverse-engineered" PIFs, which is simply the attributable mort/incidence 
# divided by total mort/incidence. What's the point when we calculate the PIFs 
...
# function is two merge totalmort and allmort (to ensure everything matches up 
# by subgroup), next is to identify column indices for the totalmort and allmort 
# simulations, and then finally use those indices to take allmort sims and divide 
# by total mort sims. Note that this function only works when your subgroups 
...
get.RE.PAFs.draws <- function(allmort, totalmort) {
...
# for now, only works for age/sex/race
get.RE.PAFs <- function(allmort, totalmort) {
...
sum.stats.maker <- function(allmort) {
  ...
  mort.summary <- 
    allmort.dt[, ":="(means = rowMeans(.SD, na.rm = TRUE),
...
# simulation output for attributable mort (allmort), total mort (total mort), 
...
Sum.by.strata <- function(allmort, totalmort, pop, covar) {
  ...
  # interest. Let's say it's age, as an example. For attributable mort/incidence, 
  ...
  strata.sims.allmort <- 
    allmort.dt[,lapply(.SD, sum), 
               by = c(strata, "riskfactor"), 
               .SDcols = cols.to.sum.allmort]
  
  strata.sims.totalmort <- 
    totalmort.dt[,lapply(.SD, sum), 
                 by = c(strata[strata %in% c("age", "female", "race", "outcome", "disease_type")]), 
  ...
  # without issue. In the other three cases, where either strata.sims.totalmort 
  ...
  # and/or if strata.sims.allmort is one dimensional 
  # (if dim(strata.sims.totalmort)[1]>1), 
  # then we need to use cbind to combine those to the rest of the files. 
  
    if(length(strata[strata %in% c("age", "female", "race")]) > 0 & 
       dim(strata.sims.totalmort)[1] > 1) {
      
      merged <- 
        merge(merge(strata.sims.allmort, strata.sims.totalmort), 
              strata.sims.pop, 
    ...
    if(length(strata[strata %in% c("age", "female", "race")]) == 0 & 
      ...
      merged <- cbind(merge(strata.sims.allmort, strata.sims.totalmort), 
                      strata.sims.pop)
    ...
    if(length(strata[strata %in% c("age", "female", "race")]) == 0 & 
      ...
      merged <- cbind(strata.sims.allmort, 
                      strata.sims.totalmort, 
                      strata.sims.pop)
  ...
  # calculate "reverse-engineered" PIFs and "standardized mort", 
  ...
  # Merge all the summary output we created (attributable mort, RE.PIF, 
  # standardized mort) into one super output file which we will call 
  # "strata.summary.stats". 
  
  #merged.results <- merge(sum.stats.attr.mort.PAF, RE.PAFs.sum.stats)
  merged.results <- reduce(list(sum.stats.attr.mort.PAF, 
                                RE.PAFs.sum.stats, 
  ...
  # 2. simulated values for attributable mort/incidence by specified strata of interest, 
...
Sum.by.strata.joint <- function(allmort, totalmort, pop ,covar) {
  ...
  strata.sims.allmort <- allmort.dt[, lapply(.SD, sum), 
                                    by = c(strata), 
                                    .SDcols = cols.to.sum.allmort]
  ...
  if(length(strata[strata %in% c("age", "female", "race")]) > 0 & 
    ...
    merged <- merge(merge(strata.sims.allmort, strata.sims.totalmort), 
                    strata.sims.pop, 
                    by = strata[strata %in% c("age", "female", "race")])
  ...
  merged.results <- 
    reduce(list(sum.stats.attr.mort.PAF, RE.PAFs.sum.stats, standardized.mort.sum.stats), 
           merge, 
           by = strata)
